...
[ "$1" = "1" ] && claude --dangerously-skip-permissions -p "Go into all step files (except 1, that's just init) and break down the existing substeps into smaller more straight forward steps"
[ "$1" = "2" ] && claude --dangerously-skip-permissions -p "Go into all step files (except 1, that's just init) and make sure that our definition of DONE is precise enough. Tests can never mock away any critical functionality. Ensure mocks are clearly for isolation, not for skipping hard tests. 100 tests passing means nothing if those tests use mocks for the critical path"
